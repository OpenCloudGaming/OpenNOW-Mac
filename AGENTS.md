...
- **Stream HUD dropdowns are `StreamHUDDropdown`, never `OPNDropdownMenu`.** The catalog menu draws its panel as a plain `.overlay` on the trigger with the catalog fill and no `StreamHUDExpandedPanelKey`, so inside the HUD's translucent sidebar the next section's background paints over the open list: the rows read as translucent text spilling across the panel below, which reaches review as "the dropdowns are buggy" rather than as a compile error. `StreamHUDDropdown` is the HUD's own control — opaque double fill, shadow, section lift through `StreamHUDExpandedPanelKey`, upward opening when there is no room below, and a capped scrolling panel for long lists. Take its rows from the model (`padDropdownItems`) so the pointer and the pad run the same action, and pass `padDriver` so a controller can open, walk and commit it. The catalog, Settings, Screenshots, Recordings and Remote Co-Op guest surfaces keep `OPNDropdownMenu`. The same trap catches any panel a HUD control opens by hand: `StreamHUDComponents.swift` must stay the only place in `View/Stream/` that draws an open panel, because publishing `StreamHUDExpandedPanelKey` is what lifts the section. Enforced by the `design_no_catalog_dropdown_in_hud` swiftlint rule over `View/Stream/`, and covered visually by `StreamHUDDropdownSnapshotTests`, which renders an open dropdown over the section beneath it.
