...
private let allDensities = OPNThemePreferences.TileDensity.allCases.map(\.tileScale)
...
@Test func compactTileIsSmallerThanComfortableAndLargeIsBiggerAtEveryInterfaceScale() {
    for scale: CGFloat in [0.75, 1.0, 1.25, 1.5, 2.0] {
        let compactWideWidth = CatalogVendorLayout.wideTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.compact.tileScale)
        let comfortableWideWidth = CatalogVendorLayout.wideTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.comfortable.tileScale)
        let largeWideWidth = CatalogVendorLayout.wideTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.large.tileScale)
        #expect(compactWideWidth < comfortableWideWidth)
        #expect(largeWideWidth > comfortableWideWidth)

        let compactWideHeight = CatalogVendorLayout.wideTileHeight(scale: scale, density: OPNThemePreferences.TileDensity.compact.tileScale)
        let comfortableWideHeight = CatalogVendorLayout.wideTileHeight(scale: scale, density: OPNThemePreferences.TileDensity.comfortable.tileScale)
        let largeWideHeight = CatalogVendorLayout.wideTileHeight(scale: scale, density: OPNThemePreferences.TileDensity.large.tileScale)
        #expect(compactWideHeight < comfortableWideHeight)
        #expect(largeWideHeight > comfortableWideHeight)

        let compactPosterWidth = CatalogPosterLayout.posterTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.compact.tileScale)
        let comfortablePosterWidth = CatalogPosterLayout.posterTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.comfortable.tileScale)
        let largePosterWidth = CatalogPosterLayout.posterTileWidth(scale: scale, density: OPNThemePreferences.TileDensity.large.tileScale)
...
@Test func aDenserSettingYieldsAtLeastAsManyColumnsAtTheSameWindowWidth() {
    for width: CGFloat in [900, 1440, 2560, 5120] {
        for scale: CGFloat in [1.0, 1.5] {
            let compactColumns = CatalogPosterLayout.columnCount(forWidth: width, scale: scale, density: OPNThemePreferences.TileDensity.compact.tileScale)
            let comfortableColumns = CatalogPosterLayout.columnCount(forWidth: width, scale: scale, density: OPNThemePreferences.TileDensity.comfortable.tileScale)
            let largeColumns = CatalogPosterLayout.columnCount(forWidth: width, scale: scale, density: OPNThemePreferences.TileDensity.large.tileScale)
...
@Test func aComfortableDensityMatchesTheUndecoratedLayoutDefaults() {
    ...
    let comfortableScale = OPNThemePreferences.TileDensity.comfortable.tileScale
