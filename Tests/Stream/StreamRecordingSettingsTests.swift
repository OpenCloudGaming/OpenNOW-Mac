...
struct StreamRecordingSettingsTests {
    ...
    func writerKeepsDecoderPixelFormatAndPicksEncodableCodec() throws {
        ...
        #expect(StreamRecorder.adaptorPixelFormat(for: nv12) == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange)
        #expect(StreamRecorder.adaptorPixelFormat(for: bgra) == kCVPixelFormatType_32BGRA)

        // H.264 hardware encode stops short of NVST's 5K, so that resolution has to be HEVC.
        #expect(StreamRecorder.videoCodec(width: 1920, height: 1080) == .h264)
        #expect(StreamRecorder.videoCodec(width: 3840, height: 2160) == .h264)
        #expect(StreamRecorder.videoCodec(width: 5120, height: 2160) == .hevc)
    }
    ...
    func automaticBitrateIsCappedAtHighResolutions() {
        func automaticBitrate(width: Int, height: Int, fps: Int) -> Int {
            ...
            let settings = StreamRecorder.videoSettings(configuration: configuration, width: width, height: height)
        ...
        #expect(automaticBitrate(width: 5120, height: 2160, fps: 120) == StreamRecorder.automaticVideoBitrateCeiling)
        ...
        let settings = StreamRecorder.videoSettings(configuration: explicit, width: 5120, height: 2160)
        let compression = settings[AVVideoCompressionPropertiesKey] as? [String: Any]
