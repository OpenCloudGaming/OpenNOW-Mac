...
@MainActor struct NativeStreamViewMouseSensitivityTests {
    private func run(_ deltas: [(CGFloat, CGFloat)], sensitivity: Double) -> (x: Int, y: Int) {
        var remainder = CGPoint.zero
        var total = (x: 0, y: 0)
        for (dx, dy) in deltas {
            let scaled = NativeStreamView.scaledMouseDelta(deltaX: dx, deltaY: dy, sensitivity: sensitivity, remainder: &remainder)
    ...
    @Test func remainderNeverExceedsOneCount() {
        var remainder = CGPoint.zero
        for _ in 0..<50 { _ = NativeStreamView.scaledMouseDelta(deltaX: 1, deltaY: 1, sensitivity: 0.3, remainder: &remainder) }
