...
private func snapshot(leftPad: ControllerTrackpadState = ControllerTrackpadState(),
                      rightPad: ControllerTrackpadState = ControllerTrackpadState()) -> ControllerInputSnapshot {
    ControllerInputSnapshot(leftPad: leftPad, rightPad: rightPad)
}
...
@Suite struct SteamControllerTrackpadMouseTranslatorTests {
    @Test func touchStartProducesNoMovement() {
        ...
        let actions = translator.translate(snapshot(rightPad: ControllerTrackpadState(x: 0.4, y: 0.2, touched: true)))
        #expect(actions.isEmpty)
    }

    @Test func rightPadDragMovesMouseWithInvertedY() {
        var translator = SteamControllerTrackpadMouseTranslator()
        _ = translator.translate(snapshot(rightPad: ControllerTrackpadState(x: 0, y: 0, touched: true)))
        let actions = translator.translate(snapshot(rightPad: ControllerTrackpadState(x: 0.1, y: 0.1, touched: true)))
    ...
    @Test func liftingFingerResetsAnchor() {
        var translator = SteamControllerTrackpadMouseTranslator()
        _ = translator.translate(snapshot(rightPad: ControllerTrackpadState(x: -0.5, y: 0, touched: true)))
        _ = translator.translate(snapshot(rightPad: ControllerTrackpadState(touched: false)))
        let retouch = translator.translate(snapshot(rightPad: ControllerTrackpadState(x: 0.5, y: 0, touched: true)))
        #expect(retouch.isEmpty)
    }

    @Test func leftPadDragScrolls() {
        var translator = SteamControllerTrackpadMouseTranslator()
        _ = translator.translate(snapshot(leftPad: ControllerTrackpadState(x: 0, y: 0, touched: true)))
        let actions = translator.translate(snapshot(leftPad: ControllerTrackpadState(x: 0, y: 0.5, touched: true)))
    ...
    @Test func padClicksMapToMouseButtons() {
        ...
        let press = translator.translate(snapshot(
            leftPad: ControllerTrackpadState(touched: true, pressed: true),
            rightPad: ControllerTrackpadState(touched: true, pressed: true)
        ))
