...
// The injection route is what lets a settings surface reach a stream it does not own. Its whole
// contract is that it either reaches the live session's own input path or says `false` — never that
// it accepts an event into nowhere.

@Test @MainActor func withNoLiveSessionThereIsNothingToInjectInto() {
...
@Test @MainActor func aLiveSessionReceivesTheSyntheticEvent() {
    let id = UUID()
...
@Test @MainActor func aSessionThatRefusesTheEventSaysSo() {
...
@Test @MainActor func aSessionWithoutAnInjectorRefusesInsteadOfCrashing() {
    let id = UUID()
...
@Test @MainActor func deactivatingASessionDropsItsInjector() {
    let id = UUID()
...
@Test @MainActor func theMostRecentSessionIsTheOneInjectedInto() {
