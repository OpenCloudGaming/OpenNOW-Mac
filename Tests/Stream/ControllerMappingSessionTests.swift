...
@Suite struct ControllerMappingSessionTests {
    ...
    @Test func unassignedNativeInputPreservesEveryBitAndAnalogValue() throws {
        var session = ControllerMappingSession(deviceID: device, playerIndex: 2)
        ...
        let events = session.process(input, now: .now, timestamp: stamp)
        #expect(events == [.gamepad(input.gamepadState(deviceID: device, playerIndex: 2, timestamp: stamp))])
        #expect(session.process(input, now: .now, timestamp: stamp).isEmpty)
    ...
    @Test func configuredControlConsumesOnlyItsOriginalButton() throws {
        let profile = ControllerMappingProfile(name: "Custom", family: .generic, bindings: [.faceA: .keyboardKey(keyCode: 49, modifiers: [])])
    ...
    @Test func unchangedHeldStickContinuesMovingAndChordSettles() {
        ...
        let first = session.process(input, now: now, timestamp: stamp)
        let second = session.process(input, now: now.advanced(by: .milliseconds(60)), timestamp: stamp)
    ...
    @Test func sharedKeyAndMouseBindingsReleaseOnlyAfterLastControl() {
        ...
        let partial = session.process(ControllerInputSnapshot(buttons: [.east, .north]), now: .now, timestamp: stamp)
        #expect(keys(partial).isEmpty)
        #expect(!partial.contains { if case .mouse(.button) = $0 { true } else { false } })
        let released = session.process(ControllerInputSnapshot(), now: .now, timestamp: stamp)
    ...
    @Test func profileReplacementAndDisableReleaseBeforeNewInput() {
        ...
        let next = session.process(input, now: .now, timestamp: stamp)
        ...
        #expect(gamepad(session.process(input, now: .now, timestamp: stamp))?.buttons == [.south])
    }
    ...
    @Test func lifecycleResetReleasesMappedOutputsAndClearsEdges() {
        ...
        #expect(keys(session.process(input, now: .now, timestamp: stamp)).map(\.isPressed) == [true])
    }
    ...
    @Test func separateIdenticalPadsDoNotShareBindingState() {
        ...
        let input = ControllerInputSnapshot(buttons: [.south])
        #expect(keys(first.process(input, now: .now, timestamp: stamp)).count == 1)
        let untouched = second.process(input, now: .now, timestamp: stamp)
        #expect(keys(untouched).isEmpty)
        #expect(gamepad(untouched)?.buttons == [.south])
        _ = first.reset(timestamp: stamp)
        #expect(second.process(input, now: .now, timestamp: stamp).isEmpty)
    }
    ...
    @Test func dualShockTouchBeginsAndEndsWithoutPhantomMotion() {
        ...
        let start = session.process(frame(0.2, touched: true), now: .now, timestamp: stamp)
        #expect(!start.contains { if case .mouse(.moved) = $0 { true } else { false } })
        let drag = session.process(frame(0.4, touched: true, pressed: true), now: .now, timestamp: stamp)
        #expect(drag.contains { if case .mouse(.moved) = $0 { true } else { false } })
        let end = session.process(frame(0.4, touched: false), now: .now, timestamp: stamp)
        ...
        let restart = session.process(frame(-0.9, touched: true), now: .now, timestamp: stamp)
