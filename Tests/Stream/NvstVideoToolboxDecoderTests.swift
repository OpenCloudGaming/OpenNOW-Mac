...
extension NvstVideoToolboxDecoderTests {
    ...
    /// Every surface the decoder can be asked for must bind as two planes: the spatial shader reads
    /// luma plus interleaved chroma at any subsampling and depth, so no decoded surface has to be
    /// converted before it is drawn.
    @Test func everyDecoderSurfaceBindsAsTwoPlanes() {
        typealias Format = NvstVideoToolboxDecoder.BitstreamFormat
