...
struct NvstControlCommandTests {
    ...
    @Test func encodesThePingBackAckKeepalive() {
        ...
        #expect([UInt8](command.encoded) == [0x00, 0x02, 0x04, 0x00, 0x04, 0x03, 0x02, 0x01])
        #expect(NvstControlCommand.pingBackIntervalSeconds == 3)
    }

    @Test func encodingRoundTripsThroughTheParser() {
        let original = NvstControlCommand(code: 0x31f, payload: Data([1, 2, 3, 4, 5]))
        let (commands, trailing) = NvstControlCommand.parse(original.encoded)
    ...
    @Test func encodesTheFrameInvalidationRange() {
        ...
        #expect([UInt8](command.encoded.prefix(4)) == [0x01, 0x03, 0x18, 0x00])
        #expect(NvstControlCommand.name(for: 0x301) == "frame-invalidation-range")
    }
    ...
    @Test func encodesTheWindowAndSystemStateChanges() {
        ...
        #expect([UInt8](window.encoded.prefix(4)) == [0x20, 0x03, 0x0c, 0x00])
        ...
        #expect(NvstControlCommand.name(for: 0x320) == "window-state-change")
        #expect(NvstControlCommand.name(for: 0x321) == "system-state-change")
    }

    @Test func namesUnknownCodesAsRaw() {
        let command = NvstControlCommand(code: 0x7fff, payload: Data())
        #expect(NvstControlCommand.name(for: 0x7fff) == nil)
