...
@Suite struct RemoteCoOpNativeNVSTAudioRelayTests {
    private final class RecordingAudioSink: OPNRemoteCoOpHostAudioSink, @unchecked Sendable {
        ...
        /// Waits for the first delivered frame.
        ///
        /// The relay fans out on its own queue rather than inline, so libwebrtc's APM and Opus encode
        /// never run on the CoreAudio render thread - where, per the call site's own comment, waiting
        /// on anything is a priority inversion. Delivery is therefore no longer synchronous with the
        /// call, and asserting on `captured` immediately just races it.
        func firstCaptured(timeout: TimeInterval = 2) -> OPNRemoteCoOpHostAudioFrame? {
            let deadline = Date().addingTimeInterval(timeout)
            while Date() < deadline {
                if let frame = captured.first { return frame }
                usleep(1_000)
            }
    ...
    @Test func floatSamplesFromTheNativeSocketReachAGuest() throws {
        ...
        let frame = try #require(sink.firstCaptured())
    ...
    @Test func samplesPastFullScaleClampInsteadOfWrapping() throws {
        ...
        let converted = samples(try #require(sink.firstCaptured()))
