...
/// One decoded frame for every guest, converted to I420 at most once.
///
/// Per-peer conversion ran the identical crop, scale and format conversion once per guest: measured
/// 1.02 ms for one guest and 3.05 ms for three, on a 5K decode at the 4K preset.
///
/// Lazy rather than eager, because guests have their own frame rates and a frame they all drop must
/// cost nothing. The first peer to forward it pays; the rest get the result.
public final class OPNRemoteCoOpSharedVideoFrame: @unchecked Sendable {
    public let sourceFrame: RTCVideoFrame
    private let lock = NSLock()
    private var converted: RTCVideoFrameBuffer?

    public init(sourceFrame: RTCVideoFrame) {
        self.sourceFrame = sourceFrame
    }

    /// The lock is held across the conversion deliberately: a second peer arriving mid-conversion
    /// waits for the result rather than starting its own, and waits no longer than that would take.
    public func i420Buffer() -> RTCVideoFrameBuffer {
        lock.lock()
        defer { lock.unlock() }
        if let converted { return converted }
        let buffer = sourceFrame.buffer is RTCI420Buffer ? sourceFrame.buffer : sourceFrame.newI420().buffer
        converted = buffer
        return buffer
    }
}
...
public protocol OPNRemoteCoOpHostVideoSink: AnyObject, Sendable {
    ...
    /// Preferred entry point: lets several guests share one conversion.
    func renderSharedVideoFrame(_ frame: OPNRemoteCoOpSharedVideoFrame)
}

public extension OPNRemoteCoOpHostVideoSink {
    func renderSharedVideoFrame(_ frame: OPNRemoteCoOpSharedVideoFrame) {
        renderVideoFrame(frame.sourceFrame)
    }
}
...
public final class OPNRemoteCoOpHostVideoRelay: @unchecked Sendable {
    ...
    public func renderVideoFrame(_ frame: RTCVideoFrame) {
        ...
        let shared = OPNRemoteCoOpSharedVideoFrame(sourceFrame: frame)
        for sink in currentSinks { sink.renderSharedVideoFrame(shared) }
    }
    ...
    public func renderPixelBuffer(_ pixelBuffer: CVPixelBuffer, presentationTime: CMTime) {
        ...
        let shared = OPNRemoteCoOpSharedVideoFrame(sourceFrame: frame)
        for sink in state.0 { sink.renderSharedVideoFrame(shared) }
    }
