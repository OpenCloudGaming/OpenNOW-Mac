...
enum OPNRemoteCoOpWebSocketCodec {
    ...
    static func decodeFrames(from buffer: inout Data) throws -> [OPNRemoteCoOpWebSocketFrame] {
        // The buffer is materialised once and walked with an offset. Decoding straight from `buffer`
        // per frame copied the whole remaining buffer into a fresh array each time and then memmoved
        // the tail down again, so a single 64 KB read packed with minimum-size frames cost hundreds
        // of megabytes of copying - on the actor every guest's signaling shares, on the machine
        // playing the game.
        let bytes = [UInt8](buffer)
        var frames: [OPNRemoteCoOpWebSocketFrame] = []
        var offset = 0
        while let (frame, consumed) = try decodeFrame(bytes, from: offset) {
            offset += consumed
            frames.append(frame)
        }
    ...
    static func decodeFrame(_ data: Data) throws -> (frame: OPNRemoteCoOpWebSocketFrame, consumed: Int)? {
        try decodeFrame([UInt8](data), from: 0)
    }

    /// Returns nil when `bytes` holds no whole frame at `offset`.
    private static func decodeFrame(_ bytes: [UInt8], from offset: Int) throws -> (frame: OPNRemoteCoOpWebSocketFrame, consumed: Int)? {
        let available = bytes.count - offset
        guard available >= 2 else { return nil }
        let isFinal = bytes[offset] & 0x80 != 0
        let rawOpcode = bytes[offset] & 0x0F
        let isMasked = bytes[offset + 1] & 0x80 != 0
        var length = UInt64(bytes[offset + 1] & 0x7F)
        var cursor = 2

        if length == 126 {
            guard available >= cursor + 2 else { return nil }
            length = (UInt64(bytes[offset + cursor]) << 8) | UInt64(bytes[offset + cursor + 1])
            cursor += 2
        } else if length == 127 {
            guard available >= cursor + 8 else { return nil }
            length = bytes[(offset + cursor)..<(offset + cursor + 8)].reduce(UInt64(0)) { ($0 << 8) | UInt64($1) }
            cursor += 8
        }
        ...
        guard available >= cursor + 4 else { return nil }
        let mask = Array(bytes[(offset + cursor)..<(offset + cursor + 4)])
        cursor += 4

        let payloadLength = Int(length)
        guard available >= cursor + payloadLength else { return nil }
        var payload = Array(bytes[(offset + cursor)..<(offset + cursor + payloadLength)])
