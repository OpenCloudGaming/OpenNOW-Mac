...
public enum OPNRemoteCoOpHostingEndpoint {
    ...
    /// Adds Cloudflare relay servers to a configuration when the host has a TURN key.
    ///
    /// Appended rather than substituted: ICE still prefers a direct pair and only falls back to the
    /// relay when nothing else works, so a LAN or tailnet guest never pays the relay's extra hop. A
    /// failure here is logged and dropped - losing the relay leaves the session direct-only, which is
    /// exactly where it was before, and is not a reason to refuse the invite.
    public static func relayAugmented(_ configuration: OPNRemoteCoOpNetworkConfiguration,
        ...
        guard !relayServers.isEmpty else {
            logger?("Remote Co-Op could not build \(credentials.provider.label) relay credentials")
            return configuration
        }
        var augmented = configuration
        augmented.iceServers = configuration.iceServers + relayServers
        logger?("Remote Co-Op added \(relayServers.first?.urls.count ?? 0) \(credentials.provider.label) relay URLs")
        return augmented
    }
    ...
    public static func make(preferences: OPNRemoteCoOpPreferences,
                            ...
                            webTransportInfo: (@Sendable () -> OPNRemoteCoOpBrowserWebTransportInfo?)? = nil) async throws -> OPNRemoteCoOpHostingSession {
