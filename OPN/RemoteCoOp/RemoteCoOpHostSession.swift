...
public actor OPNRemoteCoOpHostSession {
    ...
    /// What `inputEnabled` was before a socket dropped. `noteGuestDisconnected` has to clear the live
    /// flag so nothing routes to an absent guest, which destroys the only record of whether the host
    /// had deliberately benched them.
    private var inputEnabledBeforeDisconnect: [UUID: Bool] = [:]
    ...
    /// `hostedSignaling` is asked for the channel and credential once the invite ID exists, because
    /// the channel is derived from it. Returning nil - which is what happens whenever no hosted key is
    /// configured - leaves the invite embedded, exactly as before.
    /// `inviteID` is a parameter because the hosted signaling channel is derived from it, and the
    /// host has to be listening on that channel before the invite that names it exists. Left to
    /// itself it is a fresh UUID, exactly as before.
    public func startInvite(inviteID: UUID = UUID(), applicationID: String = "", title: String = "", joinBaseURL: URL? = nil, signalingServerURL: String = "", lifetimeSeconds: TimeInterval = 3_600,
                            hostedSignaling: (@Sendable (UUID, Date) -> OPNRemoteCoOpInviteSignaling?)? = nil) throws -> OPNRemoteCoOpInvite {
        ...
        let signaling = hostedSignaling?(inviteID, expiresAt)
        let payload = OPNRemoteCoOpInviteTokenPayload(
            ...
            signalingKind: signaling == nil ? .embedded : .hosted,
            signalingChannel: signaling?.channel,
    ...
    public func registerGuest(displayName: String, inviteToken: String, participantID: UUID = UUID(), now: Date = Date()) async throws -> OPNRemoteCoOpParticipant {
        ...
        if let index = participants.firstIndex(where: { $0.id == participantID }) {
            if participants[index].connectionState == .disconnected {
                ...
                let hadInput = inputEnabledBeforeDisconnect[participantID] ?? (participants[index].playerIndex != nil)
                inputEnabledBeforeDisconnect[participantID] = nil
                participants[index].inputEnabled = participants[index].playerIndex != nil && hadInput
                participants[index].lastActivityAt = now
                let restored = participants[index]
                await inputRouter.upsertParticipant(restored)
                // The new socket numbers its packets from 1 again, so the mark left by the old one
                // would reject every one of them as stale.
                await inputRouter.resetSequence(for: participantID)
                return restored
            }
    ...
    public func approveParticipant(_ id: UUID) async throws -> OPNRemoteCoOpParticipant {
        ...
        // Resolved before anything is mutated, because it throws when no pad slot is free.
        //
        // `connectionState` used to be set first, so a throw here left the guest `.connected` with
        // `inputEnabled` still false - and the media session decides eligibility on `connectionState`
        // alone. A guest whose approval failed was therefore sent the game, behind their own "waiting
        // for approval" overlay, while the host saw nothing: the pad topology and the connected-slot
        // count both filter on `inputEnabled`, so no indicator moved.
        let resolvedPlayerIndex: Int
        if let existing = participants[index].playerIndex {
            resolvedPlayerIndex = existing
        } else {
            resolvedPlayerIndex = try nextAvailablePlayerIndex(excludingParticipantID: id)
        }
        participants[index].connectionState = .connected
        participants[index].playerIndex = resolvedPlayerIndex
    ...
    /// Sets one guest's stream quality, or clears it back to the session default with nil.
    ///
    /// Does not touch `lastActivityAt`: that drives the disconnect grace period and means "this guest
    /// is still there", which the host adjusting a setting is no evidence of.
    public func setQualityPreset(_ preset: OPNRemoteCoOpQualityPreset?, for id: UUID) async throws -> OPNRemoteCoOpParticipant {
        guard let index = participants.firstIndex(where: { $0.id == id }) else { throw OPNRemoteCoOpHostSessionError.participantNotFound }
        participants[index].qualityPreset = preset
        let participant = participants[index]
        await inputRouter.upsertParticipant(participant)
        return participant
    }

    /// Records what a guest asked for themselves. Never raises them: `effectiveQualityPreset` takes
    /// the lower of this and the host's allowance, so this is a request and the clamp is downstream.
    public func setGuestRequestedQualityPreset(_ preset: OPNRemoteCoOpQualityPreset?, for id: UUID) async throws -> OPNRemoteCoOpParticipant {
        guard let index = participants.firstIndex(where: { $0.id == id }) else { throw OPNRemoteCoOpHostSessionError.participantNotFound }
        participants[index].guestRequestedQualityPreset = preset
        let participant = participants[index]
        await inputRouter.upsertParticipant(participant)
        return participant
    }
    ...
    public func setInputEnabled(_ enabled: Bool, for id: UUID) async throws -> OPNRemoteCoOpParticipant {
        ...
        // A guest benched while their socket is down must come back benched: the reconnect path
        // restores from this record, so leaving it at its pre-disconnect value silently undid the
        // host's decision.
        if participants[index].connectionState == .disconnected { inputEnabledBeforeDisconnect[id] = enabled }
        participants[index].lastActivityAt = Date()
        let participant = participants[index]
        await inputRouter.upsertParticipant(participant)
        return participant
    ...
    public func noteGuestDisconnected(_ id: UUID, now: Date = Date()) async -> [UserInputEvent] {
        ...
        inputEnabledBeforeDisconnect[id] = participants[index].inputEnabled
    ...
    private func removeStaleDisconnectedParticipants(now: Date) {
        ...
        // Dropped with the participant. The record only exists to survive a reconnect, so keeping it
        // past the grace period both grew the dictionary for the life of the process and let a stale
        // value override a decision the host made while the guest was away.
        for id in stale { inputEnabledBeforeDisconnect[id] = nil }
    }
