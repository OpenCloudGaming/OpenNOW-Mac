...
@MainActor
final class OPNReleaseHistoryStore: ObservableObject {
    static let shared = OPNReleaseHistoryStore()
    ...
    private var updater: OPNGitHubUpdater?
    private var loadTask: Task<Void, Never>?

    private init() {}

    func attach(_ updater: OPNGitHubUpdater) {
    ...
    func reload() {
        ...
        loadTask = Task { [weak self] in
            do {
                ...
                self?.state = .loaded(summaries.map { Entry(summary: $0, notes: OPNReleaseNotesFormatter.parse($0.releaseNotes)) })
