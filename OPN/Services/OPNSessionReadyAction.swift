    ...
    static func prepareAuthorizationIfNeeded() {
        ...
        Task {
            ...
            do {
                ...
                OPNLog.info(.app, "Notification authorization \(granted ? "granted" : "denied")")
            } catch {
                OPNLog.warning(.app, "Notification authorization failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    private static func postNotification(title: String) {
        ...
        Task {
            ...
            guard status == .authorized || status == .provisional else {
                OPNLog.info(.app, "Session ready notification skipped: authorization status \(status.rawValue)")
                return
            }
            ...
            do {
                ...
                OPNLog.info(.app, "Session ready notification posted for \(gameTitle.isEmpty ? "the session" : gameTitle)")
            } catch {
                OPNLog.warning(.app, "Session ready notification failed: \(error.localizedDescription)")
            }
        }
    }
