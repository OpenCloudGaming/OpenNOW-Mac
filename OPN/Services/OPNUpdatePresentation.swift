...
final class OPNUpdatePresentation: ObservableObject {
    ...
    enum InstallState: Equatable {
        case idle
        case downloading(OPNUpdateDownloadProgress)
        case staging
    ...
    var installHandler: ((OPNGitHubRelease) -> Void)?
    ...
    private func presentRequest(_ request: Request) {
        if case .available(let release) = request {
            ...
            notes = OPNReleaseNotesFormatter.parse(release.releaseNotes)
        } else {
            notes = .empty
        }
        installState = .idle
    ...
    func install() {
        ...
        installState = .downloading(OPNUpdateDownloadProgress(receivedBytes: 0, expectedBytes: release.assetByteCount))
        #if DEBUG
    ...
    func reportDownloadProgress(_ progress: OPNUpdateDownloadProgress) {
...
#if DEBUG
extension OPNUpdatePresentation {
    ...
    * decode SRTP 31x faster by batching GHASH blocks ([bf34a18](https://github.com/OpenCloudGaming/OpenNOW-Mac/commit/bf34a18af423b3fb91d505ea0eb1a79903e2a003))
    ...
    static func sampleRelease(version: String = "0.3.0") -> OPNGitHubRelease {
        OPNGitHubRelease(
    ...
    private func runSimulatedInstall(_ release: OPNGitHubRelease) {
        simulatedInstallTask?.cancel()
        simulatedInstallTask = Task { [weak self] in
            ...
            for step in 1...steps {
                ...
                reportDownloadProgress(OPNUpdateDownloadProgress(receivedBytes: received, expectedBytes: release.assetByteCount))
            }
