...
extension OPNStreamSessionCoordinator {
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            Task { @MainActor [weak self] in
                ...
                client.connect { [weak self] success, error in
                    ...
                    self.resumeOffer(error: OPNStreamSessionError.signalingFailed(error.isEmpty ? "Unable to connect signaling." : error))
                }
            }
        }
    }
    ...
    func installSignalingHandlers(_ client: NVSTWebSocketSignalingClient,
        ...
        self.resumeOffer(error: OPNStreamSessionError.signalingFailed(reason.isEmpty ? "Signaling connection closed before receiving an offer." : reason))
    }
    }
    ...
    func startOfferTimeout(client: NVSTWebSocketSignalingClient, descriptor: StreamSessionDescriptor) {
        Task { @MainActor [weak self, weak client] in
            ...
            client.disconnect()
            self.resumeOffer(error: OPNStreamSessionError.signalingFailed("Signaling connected but no stream offer was received within 20 seconds for session \(descriptor.id)."))
        }
    }
