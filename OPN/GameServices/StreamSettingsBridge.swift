...
func streamDeviceCapabilities(from capabilities: OPNStreamDeviceCapabilities) -> StreamDeviceCapabilities {
    let connectedGamepads = NativeGamepadMonitor.connectedGamepadCount()
    let reservedRemoteGamepads = OPNRemoteCoOpPreferencesStore.reservedControllerSlotsForLaunch()
    return StreamDeviceCapabilities(
...
func streamCloudVariables(from variables: OPNStreamCloudVariables) -> StreamCloudVariables {
    StreamCloudVariables(
...
func streamProfile(from profile: OPNStreamPreferenceProfile) -> StreamProfile {
    StreamProfile(
        resolution: StreamResolution(width: profile.resolution.width, height: profile.resolution.height),
        fps: profile.fps,
        ...
        steamBigPictureMode: profile.steamBigPictureMode,
        ...
        pillarboxFillMode: profile.pillarboxFillModeIndex,
        ...
        cursorPolicy: profile.cursorPolicyIndex,
...
func streamProfile(from settings: [String: Any]) -> StreamProfile {
    let resolutionParts = bridgeString(settings["resolution"], fallback: "1920x1080").split(separator: "x").compactMap { Int($0) }
    return StreamProfile(
        resolution: StreamResolution(width: resolutionParts.first ?? 1920, height: resolutionParts.count > 1 ? resolutionParts[1] : 1080),
        ...
        enablePersistingInGameSettings: bridgeBool(settings["enablePersistingInGameSettings"]),
        vsyncMode: bridgeInt(settings["vsyncMode"], fallback: 2),
        ...
        steamBigPictureMode: bridgeInt(settings["appLaunchMode"], fallback: 1) == 2,
        ...
        pillarboxFillMode: bridgeInt(settings["pillarboxFillMode"]),
        pillarboxFillDim: bridgeInt(settings["pillarboxFillDim"], fallback: 55),
        pillarboxFillColor: bridgeString(settings["pillarboxFillColor"], fallback: "#000000"),
        ...
        rawMouseInput: bridgeBool(settings["rawMouseInput"]),
        cursorPolicy: bridgeInt(settings["cursorPolicy"]),
