...
private final class OPNSessionStopCompletion: @unchecked Sendable {
    let lock = NSLock()
...
public final class OPNStreamSessionCoordinator: StreamSessionProvider, StreamSignalingChannel, StreamSessionStartCancellable, @unchecked Sendable {
    ...
    private func stopCloudMatchSession(_ session: StreamSessionDescriptor) async -> Error? {
        await withCheckedContinuation { continuation in
            let completion = OPNSessionStopCompletion(continuation: continuation)
            DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + 10) {
                completion.resume(OPNStreamSessionError.sessionStopFailed("Cloud session stop timed out."))
            }
            OPNActiveSessionService.stopSession(
                ...
                if success || (session.metadata["accessToken"] ?? "").isEmpty {
                    ...
                    completion.resume(OPNStreamSessionError.sessionStopFailed(error.isEmpty ? "Unable to stop stream session." : error))
                }
            }
        }
    }
