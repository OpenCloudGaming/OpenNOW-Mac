...
extension OPNStreamSessionCoordinator {
    ...
    func createSession(configuration: StreamLaunchConfiguration, settings: [String: Any]) async throws -> AllocatedStreamSession {
        ...
        if success {
            ...
            throw OPNStreamSessionError.activeSessionConflict(StreamSessionConflict(
            ...
            throw OPNStreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to allocate stream session." : error)
        }
    }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            sessionManager.claimSession(sessionId: configuration.resumeSessionID, serverIp: configuration.resumeServer, appId: configuration.applicationID, settings: settings, recoveryMode: isRecovery) { success, info, error in
                if success {
                    ...
                    continuation.resume(throwing: OPNStreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to resume stream session." : error))
                }
            }
        }
    }
    ...
    func waitForReadySession(_ initial: AllocatedStreamSession, configuration: StreamLaunchConfiguration) async throws -> AllocatedStreamSession {
        ...
        guard !initial.sessionId.isEmpty, !initial.serverIp.isEmpty else {
            throw OPNStreamSessionError.sessionAllocationFailed("Cloud session is missing session id or server address.")
        }
        ...
        while !latest.isReady {
            ...
            if attempts >= 60, !lastPollWasPendingProgress {
                throw OPNStreamSessionError.sessionAllocationFailed("Session poll timeout")
            }
            ...
            if latest.status > 3, ![4, 5, 6].contains(latest.status) {
                throw OPNStreamSessionError.sessionAllocationFailed("Session in terminal error state")
            }
    ...
    func playRequiredAd(_ ad: AllocatedSessionAd, session: AllocatedStreamSession) async throws -> AllocatedStreamSession {
        guard !ad.mediaUrl.isEmpty else {
            throw OPNStreamSessionError.sessionAllocationFailed("GeForce NOW requires an ad before launch, but no playable ad media was returned.")
        }
        guard let adPresenter else {
            throw OPNStreamSessionError.sessionAllocationFailed("GeForce NOW requires an ad before launch, but the loading screen ad player is not available.")
        }
    ...
    func reportSessionAd(session: AllocatedStreamSession, ad: AllocatedSessionAd, action: String, watchedTimeInMs: Int, cancelReason: String) async throws -> AllocatedStreamSession {
        ...
        if success {
            ...
            throw OPNStreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to update required ad state." : error)
        }
    }
    ...
    func pollSession(sessionId: String, serverIp: String) async throws -> AllocatedStreamSession {
        ...
        if success {
            ...
            throw OPNStreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to poll stream session." : error)
        }
    }
