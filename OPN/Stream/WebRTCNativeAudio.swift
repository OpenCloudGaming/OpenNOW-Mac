...
final class OPNLibWebRTCAudio: NSObject, @unchecked Sendable {
    ...
    func refreshAudioDevices(sessionImpl: OPNLibWebRTCSessionImpl?) {
        ...
        guard audioMonitoringActive else {
            OPNStreamTelemetry.capture("webrtc.native.audio.refresh_skipped", level: .debug, message: "Audio device refresh skipped because monitoring is inactive.")
            return
        }
        guard let sessionImpl, sessionImpl.peerConnection != nil else {
            OPNStreamTelemetry.capture("webrtc.native.audio.refresh_skipped", level: .debug, message: "Audio device refresh skipped because the peer connection is missing.")
            return
        }
        if let audioDevice = sessionImpl.audioDevice {
            audioDevice.handleDefaultDeviceChange()
            OPNStreamTelemetry.capture("webrtc.native.audio.refresh_delegated", level: .debug, message: "Audio device refresh delegated to CoreAudio RTC device.", attributes: ["inputDevice": String(defaultInputDevice), "outputDevice": String(defaultOutputDevice)])
            return
        }
        ...
        OPNStreamTelemetry.capture("webrtc.native.audio.refresh_scheduled", level: .debug, message: "Audio device refresh scheduled.", attributes: ["inputDevice": String(defaultInputDevice), "outputDevice": String(defaultOutputDevice)])
        ...
        Task { @MainActor [weak self, weak sessionImpl] in
            ...
            OPNStreamTelemetry.capture("webrtc.native.audio.refresh_applied", level: .debug, message: "Audio device refresh applied.", attributes: ["inputDevice": String(self.defaultInputDevice), "outputDevice": String(self.defaultOutputDevice), "remoteTrack": String(sessionImpl?.remoteAudioTrack != nil), "microphoneTrack": String(sessionImpl?.localMicrophoneTrack != nil), "microphoneEnabled": String(sessionImpl?.localMicrophoneTrack?.isEnabled == true)])
        }
    }
    ...
    @objc func startAudioDeviceMonitoring() {
        ...
        OPNStreamTelemetry.capture("webrtc.native.audio.monitor.start", level: .debug, message: "Audio device monitoring started.", attributes: ["devicesStatus": String(devicesStatus), "inputStatus": String(inputStatus), "outputStatus": String(outputStatus), "inputDevice": String(defaultInputDevice), "outputDevice": String(defaultOutputDevice)])
    }
    ...
    @objc func stopAudioDeviceMonitoring() {
        ...
        OPNStreamTelemetry.capture("webrtc.native.audio.monitor.stop", level: .debug, message: "Audio device monitoring stopped.")
    }
    ...
    func handleAudioDeviceChange(sessionImpl: OPNLibWebRTCSessionImpl?) {
        ...
        if outputDevice == AudioDeviceID(kAudioObjectUnknown) {
            ...
            if audioDeviceUnavailableRetryCount < 10 {
                ...
                OPNStreamTelemetry.capture("webrtc.native.audio.output_unavailable", level: .debug, message: "Default output device unavailable during hotplug; retrying.", attributes: ["inputDevice": String(inputDevice), "outputDevice": String(outputDevice), "retry": String(audioDeviceUnavailableRetryCount)])
                Task { @MainActor [weak self] in
                ...
                OPNStreamTelemetry.capture("webrtc.native.audio.output_unavailable", level: .warning, message: "Default output device remained unavailable after hotplug retries.")
            }
            return
        }
        ...
        OPNStreamTelemetry.capture("webrtc.native.audio.device_changed", level: .info, message: "Default audio device changed.", attributes: ["previousInput": String(defaultInputDevice), "inputDevice": String(inputDevice), "previousOutput": String(defaultOutputDevice), "outputDevice": String(outputDevice)])
        ...
        if !customAudioDeviceActive, Self.envFlagEnabled("OPN_ENABLE_WEBRTC_AUDIO_HOTSWAP_RECOVERY", defaultValue: true) {
            Task { @MainActor [weak self] in
                ...
                guard let self, self.audioMonitoringActive, self.audioDeviceChangeGeneration == generation else { return }
                OPNStreamTelemetry.capture("webrtc.native.audio.recovery", level: .warning, message: "Forcing stream recovery after audio device change.", attributes: ["inputDevice": String(self.defaultInputDevice), "outputDevice": String(self.defaultOutputDevice)])
    ...
    func startMicrophoneLevelPolling(sessionImpl: OPNLibWebRTCSessionImpl?, statsQueue: DispatchQueue) {
        ...
        timer.setEventHandler { [weak self, weak sessionImpl] in
            ...
            if self.microphoneLevelRequestInFlight {
                ...
                OPNStreamTelemetry.capture("webrtc.native.audio.microphone_level.stalled", level: .warning, message: "Microphone level request never completed; retrying.")
            }
        ...
        timer.resume()
        OPNStreamTelemetry.capture("webrtc.native.audio.microphone_level", level: .debug, message: "Microphone level polling started.")
    }
