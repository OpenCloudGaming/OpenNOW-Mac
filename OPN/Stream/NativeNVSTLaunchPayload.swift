...
struct NativeNVSTLaunchPayload: Equatable, Sendable {
    ...
    init(allocation: NativeNVSTSessionAllocation, streamingProfileJSON: String, clientAppVersion: String) {
        ...
        let tokenType = Self.firstNonEmpty(Self.string(rawSession["tokenType"]), Self.string(rawSession["authType"]), Self.string((rawSession["auth"] as? [String: Any])?["type"]))
        let token = Self.firstNonEmpty(Self.string(rawSession["token"]), Self.string(rawSession["authToken"]), Self.string(rawSession["jwt"]), Self.string((rawSession["auth"] as? [String: Any])?["token"]), Self.string(rawSession["sessionToken"]))
