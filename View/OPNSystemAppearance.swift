...
/// own.
@MainActor
final class OPNSystemAppearance: ObservableObject {
    @Published private(set) var isDark = OPNSystemAppearance.isSystemDark()
    ...
    init() {
        observation = NSApplication.shared.observe(\.effectiveAppearance, options: [.new]) { [weak self] _, _ in
            Task { @MainActor [weak self] in
                self?.isDark = OPNSystemAppearance.isSystemDark()
            }
        }
    }
