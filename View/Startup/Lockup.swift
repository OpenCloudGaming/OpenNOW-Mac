...
struct StartupLockup: View {
    ...
    var body: some View {
        ...
        VStack(spacing: (metrics.compact ? 16 : 22) * scale) {
            StartupLogoCore(stage: stage, metrics: metrics)

            StartupWordmark(stage: stage, metrics: metrics)
        }
        .frame(width: metrics.lockupWidth, height: metrics.bandHeight)
        .mask(alignment: .top) {
            StartupDevelopMask(reveal: reveal)
        }
...
private struct StartupDevelopMask: View {
    let reveal: Double
...
private struct StartupLogoCore: View {
    let stage: StartupStage
    let metrics: StartupMetrics
            ...
            HStack(spacing: spacing) {
                ForEach(Array(Self.letters.enumerated()), id: \.offset) { index, letter in
                    ...
                    if letter == " " {
                        ...
                        StartupWordmarkLetter(letter: letter, size: size, reveal: reveal, scale: scale)
                    }
                }
            }

            Text("CLOUD GAMING CLIENT")
                .font(OPNDesign.Typography.mono(size: metrics.compact ? 9 : 11, scale: scale, weight: .bold))
                .tracking((3.4 + 3.4 * (1 - settle)) * scale)
                .foregroundStyle(OPNDesign.accentInk.opacity(0.92 * settle))
                .opacity(settle)
        }
    }
}
...
private struct StartupWordmarkLetter: View {
    ...
    private var glyph: some View {
        Text(String(letter))
            .font(OPNDesign.Typography.display(size: size))
