...
struct StartupRail: View, Equatable {
    ...
    nonisolated static func == (lhs: StartupRail, rhs: StartupRail) -> Bool {
        ...
        VStack(spacing: 10 * scale) {
            HStack(alignment: .firstTextBaseline) {
                Text(stage.statusText)
                    .font(OPNDesign.Typography.label(size: metrics.compact ? 10 : 12, scale: scale, weight: .black))
                ...
                Text("\(percent)%")
                    .font(OPNDesign.Typography.mono(size: metrics.compact ? 10 : 12, scale: scale, weight: .black))
...
private struct StartupSegmentBar: View {
    let filledCount: Int
    let cells: Int
    let scale: CGFloat
