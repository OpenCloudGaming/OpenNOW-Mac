...
struct OPNReleaseNotesView: View {
    ...
    private func sectionView(_ section: OPNReleaseNotes.Section) -> some View {
    ...
    private func sectionHeader(_ section: OPNReleaseNotes.Section) -> some View {
    ...
    private func entryRow(_ entry: OPNReleaseNotes.Entry) -> some View {
    ...
    private func paragraphText(_ paragraph: String) -> some View {
        Text(OPNReleaseNotesFormatter.attributedText(paragraph))
            .font(.uiSans(size: metrics.entrySize * uiScale, weight: .medium))
            .foregroundStyle(OPNDesign.Text.secondary)
            .tint(OPNDesign.accent)
            .lineSpacing(2 * uiScale)
    ...
    private func visibleEntries(of section: OPNReleaseNotes.Section) -> [OPNReleaseNotes.Entry] {
    ...
    private func hiddenEntryCount(of section: OPNReleaseNotes.Section) -> Int? {
...
private struct ReleaseNoteChip: View {
    ...
    private var chipLabel: some View {
        Text(label)
            ...
            .foregroundStyle(isHovering ? OPNDesign.Text.secondary : OPNDesign.Text.muted)
            .tracking(0.7)
            .lineLimit(1)
