...
extension RemoteCoOpSettingsPage {
    ...
    func relayTestTag(_ passed: Bool) -> some View {
        Text(passed ? "SUCCESS" : "FAILED")
            ...
            .background(passed ? OPNDesign.accent : OPNDesign.Semantic.destructive)
    }

    var relayTestHintColor: Color {
        guard let outcome = relayTestOutcome else { return OPNDesign.Text.tertiary }
        return outcome ? OPNDesign.accent : OPNDesign.Semantic.destructive
    }
