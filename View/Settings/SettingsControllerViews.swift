    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 16 * uiScale) {
            ...
            SettingsCard(title: "Streaming Quality", uiScale: uiScale) {
                ...
                SettingsDivider(uiScale: uiScale)
                SettingsToggleRow(title: "16:9 Titles at 16:9", subtitle: "Games seen rendering 16:9 inside a wider stream are requested at the 16:9 resolution of the same height next launch, so the black bars are no longer encoded, sent and decoded. Applies while Pillarbox Fill is Black.", isOn: viewModel.streamProfile.streamSixteenNineTitlesAtSixteenNine, uiScale: uiScale, action: viewModel.setStreamSixteenNineTitlesAtSixteenNine)
                SettingsDivider(uiScale: uiScale)
