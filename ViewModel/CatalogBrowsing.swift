...
extension CatalogViewModel {
    ...
    func browseCatalog(forceRefresh: Bool) {
        ...
        gameService.browseCatalogObject(
            ...
            guard success else {
                OPNLog.warning(.catalog, "Show All browse failed elapsed=\(elapsedMs)ms error=\(error)")
            ...
            if isPartialDelivery {
                OPNLog.info(.catalog, "Show All first page delivered elapsed=\(elapsedMs)ms games=\(newCount) total=\(browseResult.totalCount) hasNext=\(browseResult.hasNextPage)")
            } else {
                OPNLog.info(.catalog, "Show All browse completed elapsed=\(elapsedMs)ms games=\(newCount) prevGames=\(resultCount) total=\(browseResult.totalCount)")
            }
            ...
            OPNLog.info(.catalog, "Show All result applied games=\(newCount) filterGroups=\(browseResult.filterGroups.count) sortOptions=\(browseResult.sortOptions.count) isPartial=\(isPartialDelivery)")
    ...
    func loadNextCatalogPage() {
        ...
        OPNLog.info(.catalog, "Show All loading next page cursor=\(catalogEndCursor)")
        gameService.browseCatalogObject(
            ...
            self.isLoadingMoreCatalog = false
            guard success else {
                OPNLog.warning(.catalog, "Show All next page failed error=\(error)")
                return
            }
            let browseResult = result
            ...
            self.hasMoreCatalogResults = browseResult.hasNextPage
            self.catalogEndCursor = browseResult.endCursor
            OPNLog.info(.catalog, "Show All next page applied added=\(newGames.count) total=\(self.catalogGames.count) hasNext=\(browseResult.hasNextPage)")
            self.schedulePatchingPollIfNeeded()
        }
    }
