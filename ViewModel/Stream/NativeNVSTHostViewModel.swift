...
final class NativeNVSTHostViewModel: ObservableObject {
    ...
    func performAutopilotAction(_ action: String) async {
        ...
        if action.hasPrefix("rsnap") {
            ...
            let directory = ProcessInfo.processInfo.environment["OPN_NVST_AUTOPILOT_SNAPSHOT_DIR"].map { URL(fileURLWithPath: $0, isDirectory: true) }
                ?? FileManager.default.temporaryDirectory
            nativeView?.nvstBifrostFreeRenderer?.requestRenderSnapshot(to: directory.appendingPathComponent("\(name).jpg"))
            return
        }
        ...
        if action.hasPrefix("snap") {
            let name = String(action.dropFirst(4)).isEmpty ? "frame" : String(action.dropFirst(4))
            let directory = ProcessInfo.processInfo.environment["OPN_NVST_AUTOPILOT_SNAPSHOT_DIR"].map { URL(fileURLWithPath: $0, isDirectory: true) }
                ?? FileManager.default.temporaryDirectory
            let url = directory.appendingPathComponent("\(name).jpg")
            if let size = nativeView?.nvstBifrostFreeRenderer?.writeLatestFrameJPEG(to: url) {
                OpenNOWLog.info(.stream, "Autopilot: snapshot \(Int(size.width))x\(Int(size.height)) -> \(url.path)")
            } else {
                OpenNOWLog.warning(.stream, "Autopilot: snapshot failed (no frame yet?) -> \(url.path)")
            }
            return
        }
        ...
        let now = { MediaTimestamp(nanoseconds: DispatchTime.now().uptimeNanoseconds) }
        if action.hasPrefix("key"), let code = UInt16(action.dropFirst(3)) {
            dispatcher.enqueue(.keyboard(KeyboardEvent(deviceID: "keyboard", keyCode: code, scanCode: code, isPressed: true, timestamp: now())))
            try? await Task.sleep(for: .milliseconds(90))
            dispatcher.enqueue(.keyboard(KeyboardEvent(deviceID: "keyboard", keyCode: code, scanCode: code, isPressed: false, timestamp: now())))
        ...
        if action.hasPrefix("click") {
            ...
            dispatcher.enqueue(.mouse(.button(deviceID: "mouse", button: .left, isPressed: true, timestamp: now())))
            try? await Task.sleep(for: .milliseconds(90))
            dispatcher.enqueue(.mouse(.button(deviceID: "mouse", button: .left, isPressed: false, timestamp: now())))
