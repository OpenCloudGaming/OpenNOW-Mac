...
extension NativeNVSTHostViewModel {
    ...
    func logRenderDiagnosticsIfDue() {
        ...
        OpenNOWLog.info(.stream, String(format: "render mode=%@ tier=%@ received=%llu drawn=%llu skipped=%llu present=%.1fms max=%.1f jitter=%.2f interval=%.1f/%.1f",
    ...
    func updateSixteenNineDetection(_ snapshot: NativeNVSTPerformanceSnapshot) {
        ...
        OPNStreamPreferences.rememberTitleStreamsSixteenNineContent(configuration.applicationID, change)
        OpenNOWLog.info(.stream, "16:9 title verdict \(change ? "16:9" : "native") in a \(snapshot.resolution) frame (bars \(sixteenNineTracker.barSamples)/\(sixteenNineTracker.totalSamples)); remembered for \(configuration.applicationID)")
