...
public enum UDSDiagnosticReportParser {
    public static func parse(_ json: [String: Any]) -> UDSDiagnosticReport {
        ...
        return UDSDiagnosticReport(
            streamedAppName: udsStringValue(report["streamedAppName"]) ?? "",
            sessionId: udsStringValue(report["sessionId"]) ?? "",
            errorCode: udsStringValue(report["errorCode"]) ?? udsStringValue(json["errorCode"]) ?? "",
...
public enum UDSEventReportParser {
    public static func parse(_ json: [String: Any]) -> UDSEventReportResult {
        UDSEventReportResult(
            ...
            status: udsStringValue(json["status"]) ?? udsStringValue(json["statusDescription"]) ?? ""
        )
    }
}
...
public struct UDSURLSessionTransport: UDSHTTPTransport {
    ...
    public func send(_ request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        try await OPNURLSessionHTTPTransport.send(request, operation: "uds.transport", invalidHTTPResponseError: UDSServiceError.invalidHTTPResponse)
    }
}
...
private func udsStringValue(_ value: Any?) -> String? {
